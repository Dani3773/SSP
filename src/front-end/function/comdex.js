...
async function apiRequest(endpoint, options = {}) {
  ...
  try {
    ...
    // Mock para câmeras
    if (endpoint === '/cameras' && options.method === 'GET') {
      return getMockCameras();
    }
    if (endpoint.startsWith('/cameras/') && !options.method) {
      const id = endpoint.split('/')[2];
      return getMockCameraById(id);
    }
    if (endpoint === '/cameras' && options.method === 'POST') {
      return saveMockCamera(JSON.parse(options.body));
    }
    if (endpoint.startsWith('/cameras/') && options.method === 'PUT') {
      const id = endpoint.split('/')[2];
      return updateMockCamera(id, JSON.parse(options.body));
    }
    if (endpoint.startsWith('/cameras/') && options.method === 'DELETE') {
      const id = endpoint.split('/')[2];
      return deleteMockCamera(id);
    }

    // Mock para denúncias
    if (endpoint === '/denuncias' && options.method === 'GET') {
      return getMockDenuncias();
    }
    if (endpoint.startsWith('/denuncias/') && !options.method) {
      const id = endpoint.split('/')[2];
      return getMockDenunciaById(id);
    }
    if (endpoint === '/denuncias' && options.method === 'POST') {
      return saveMockDenuncia(JSON.parse(options.body));
    }
    if (endpoint.startsWith('/denuncias/') && options.method === 'PUT') {
      const id = endpoint.split('/')[2];
      return updateMockDenuncia(id, JSON.parse(options.body));
    }
    if (endpoint.startsWith('/denuncias/') && options.method === 'DELETE') {
      const id = endpoint.split('/')[2];
      return deleteMockDenuncia(id);
    }
    if (endpoint.startsWith('/denuncias/') && options.method === 'PATCH') {
      const parts = endpoint.split('/');
      const id = parts[2];
      // PATCH /denuncias/:id/status
      if (parts[3] === 'status') {
        const body = JSON.parse(options.body || '{}');
        return changeMockDenunciaStatus(id, body.status);
      }
    }
...
// ==============================
// Tabela: estado e paginação
// ==============================
const TABLE_STATE = {
  news: { items: [], page: 1, pageSize: 10, filter: '' },
  denuncias: { items: [], page: 1, pageSize: 10, filter: '' },
  cameras: { items: [], page: 1, pageSize: 10, filter: '' },
  users: { items: [], page: 1, pageSize: 10, filter: '' },
  analyses: { items: [], page: 1, pageSize: 10, filter: '' }
};
...
function renderPaginationControls(section, totalItems) {
  const state = TABLE_STATE[section];
  ...
  if (!container) {
    ...
    const table = document.getElementById(`${section}-table-body`);
    if (table && table.parentElement) {
      table.parentElement.parentElement.appendChild(container);
    }
  }
  ...
  // Construir controles simples: anterior / próximo / página atual
  container.innerHTML = `
    <div class="pagination-controls">
      <button class="btn-secondary" ${currentPage <= 1 ? 'disabled' : ''} onclick="changePage('${section}', ${currentPage - 1})">&larr; Anterior</button>
      <span class="pagination-info">Página ${currentPage} de ${totalPages}</span>
      <button class="btn-secondary" ${currentPage >= totalPages ? 'disabled' : ''} onclick="changePage('${section}', ${currentPage + 1})">Próxima &rarr;</button>
      <select onchange="changePageSize('${section}', this.value)" class="page-size-select">
        ...
        <option value="-1" ${state.pageSize==-1? 'selected':''}>Todos</option>
      </select>
    </div>
  `;
}
...
function changePage(section, page) {
  const state = TABLE_STATE[section];
  if (!state) return;
  const totalItems = (state.items || []).filter(i => itemMatchesFilter(i, state.filter)).length;
  const totalPages = Math.max(1, Math.ceil(totalItems / state.pageSize));
  state.page = Math.min(Math.max(1, page), totalPages);
  // Chamar render por seção
  switch (section) {
    case 'news': renderNewsTable(state.items); break;
    case 'denuncias': renderDenunciasTable(state.items); break;
    case 'cameras': renderCamerasTable(state.items); break;
    case 'users': renderUsersTable(state.items); break;
    case 'analyses': renderAnalysisTable(state.items); break;
  }
}

function changePageSize(section, newSize) {
  const state = TABLE_STATE[section];
  if (!state) return;
  state.pageSize = parseInt(newSize, 10) || 10;
  if (state.pageSize === -1) state.pageSize = Number.MAX_SAFE_INTEGER; // Mostrar todos
  state.page = 1;
  changePage(section, 1);
}

// Checa se um item corresponde ao filtro atual (procura em todas as propriedades de texto)
function itemMatchesFilter(item, filter) {
  ...
  try {
    for (const key in item) {
      ...
      if (typeof val === 'number' && String(val).includes(f)) return true;
      if (typeof val === 'object' && JSON.stringify(val).toLowerCase().includes(f)) return true;
    }
    return false;
  } catch (e) {
    return false;
...
function initializeMockData() {
  ...
  if (stored) {
    ...
    mockNewsData = getDefaultMockNews();
    // saveMockDataToStorage() will be called after we initialize all mocks below
  }

  // Inicializar câmeras mock
  const storedCams = localStorage.getItem('ssp-mock-cameras');
  if (storedCams) {
    try {
      mockCamerasData = JSON.parse(storedCams);
    } catch (e) {
      console.warn('Erro ao carregar câmeras mock do localStorage:', e);
      mockCamerasData = getDefaultMockCameras(120);
    }
  } else {
    mockCamerasData = getDefaultMockCameras(120);
  }

  // Inicializar denúncias mock
  const storedDen = localStorage.getItem('ssp-mock-denuncias');
  if (storedDen) {
    try {
      mockDenunciasData = JSON.parse(storedDen);
    } catch (e) {
      console.warn('Erro ao carregar denúncias mock do localStorage:', e);
      mockDenunciasData = getDefaultMockDenuncias(200);
    }
  } else {
    mockDenunciasData = getDefaultMockDenuncias(200);
  }
...
function getDefaultMockNews(count = 20) {
  const categories = ['noticia','comunicado','alerta','informacao'];
  ...
  for (let i = 1; i <= count; i++) {
    ...
    news.push({
      id: i,
      title: `Notícia de teste ${i} - ${['Instalação','Atualização','Aviso'][i%3]}`,
      description: `Descrição detalhada da notícia ${i}. Conteúdo de teste para gerar volume de dados no ambiente.`,
      image: i % 3 === 0 ? 'https://via.placeholder.com/300x200' : '',
...
function saveMockDataToStorage() {
  try {
    ...
    localStorage.setItem('ssp-mock-cameras', JSON.stringify(mockCamerasData));
    localStorage.setItem('ssp-mock-denuncias', JSON.stringify(mockDenunciasData));
  } catch (e) {
...
// Mock Câmeras
// ==============================
function getDefaultMockCameras(count = 100) {
  const cameras = [];
  const types = ['PTZ', 'Dome', 'Bullet', 'Fixed'];
  const statuses = ['online', 'offline', 'maintenance'];
  for (let i = 1; i <= count; i++) {
    const id = i;
    cameras.push({
      id,
      name: `Câmera ${id} - Ponto ${Math.ceil(id/3)}`,
      location: `Bairro ${['Centro','Pinheirinho','Santa Luzia','Santo Antonio'][id % 4]}`,
      latitude: -29.1 + (Math.random() * 0.2 * (i/100)),
      longitude: -49.6 + (Math.random() * 0.2 * (i/100)),
      type: types[id % types.length],
      status: statuses[id % statuses.length],
      resolution: id % 2 === 0 ? '1080p' : '720p',
      streamUrl: '',
...
function getMockCameras() {
  if (!mockCamerasData || mockCamerasData.length === 0) {
    mockCamerasData = getDefaultMockCameras(120);
  }
  return [...mockCamerasData];
}

function getMockCameraById(id) {
  const camera = mockCamerasData.find(c => c.id == id);
  if (!camera) throw new Error('Câmera não encontrada');
  return { ...camera };
}

function saveMockCamera(cameraData) {
  const newId = Math.max(...mockCamerasData.map(c => c.id), 0) + 1;
  const newCamera = {
    ...cameraData,
    id: newId,
    createdAt: new Date().toISOString()
  };
  mockCamerasData.push(newCamera);
  saveMockDataToStorage();
  return newCamera;
}

function updateMockCamera(id, updateData) {
  const index = mockCamerasData.findIndex(c => c.id == id);
  if (index === -1) throw new Error('Câmera não encontrada');
  mockCamerasData[index] = { ...mockCamerasData[index], ...updateData, updatedAt: new Date().toISOString() };
  saveMockDataToStorage();
  return mockCamerasData[index];
}

function deleteMockCamera(id) {
  const index = mockCamerasData.findIndex(c => c.id == id);
  if (index === -1) throw new Error('Câmera não encontrada');
  mockCamerasData.splice(index, 1);
  saveMockDataToStorage();
  return { success: true };
}
...
// Mock Denúncias
// ==============================
function getDefaultMockDenuncias(count = 200) {
  const denuncias = [];
  const tipos = ['furto', 'roubo', 'vandalismo', 'agressao', 'transito', 'suspeita', 'outro'];
  const prioridades = ['baixa', 'media', 'alta'];
  const statuses = ['pendente','analisando','resolvida','arquivada'];
  for (let i = 1; i <= count; i++) {
    const id = i;
    const createdAt = new Date(Date.now() - (i * 3600000));
    const data = new Date(createdAt.getTime() - ((i % 5) * 3600000));
    denuncias.push({
      id,
      titulo: `Ocorrência ${id} - ${tipos[id % tipos.length]}`,
      camera: `Câmera ${((id % 120) + 1)} - Ponto ${Math.ceil(((id % 120) + 1)/3)}`,
      tipoOcorrencia: tipos[id % tipos.length],
      dataOcorrencia: data.toISOString().split('T')[0],
      horaOcorrencia: data.toTimeString().split(' ')[0].substr(0,5),
      localizacao: `Rua ${Math.ceil(id/5)}, Bairro ${['Centro','Pinheirinho','Santa Luzia','Santo Antonio'][id % 4]}`,
      descricao: `Descrição da ocorrência ${id}. Relato de atividade suspeita...`,
      suspeitos: id % 3 === 0 ? `Indivíduo com camiseta ${['vermelha','preta','branca'][id%3]}` : '',
      prioridade: prioridades[id % prioridades.length],
      status: statuses[id % statuses.length],
      urgente: (id % 10) === 0,
      origem: 'mock',
      createdAt: createdAt.toISOString(),
      dataHora: createdAt.toISOString()
    });
  }
  return denuncias;
}

function getMockDenuncias() {
  if (!mockDenunciasData || mockDenunciasData.length === 0) {
    mockDenunciasData = getDefaultMockDenuncias(200);
  }
  return [...mockDenunciasData];
}

function getMockDenunciaById(id) {
  const den = mockDenunciasData.find(d => d.id == id);
  if (!den) throw new Error('Denúncia não encontrada');
  return { ...den };
}

function saveMockDenuncia(denunciaData) {
  const newId = Math.max(...mockDenunciasData.map(d => d.id), 0) + 1;
  const now = new Date().toISOString();
  const newDen = { ...denunciaData, id: newId, createdAt: now, dataHora: now };
  mockDenunciasData.push(newDen);
  saveMockDataToStorage();
  return newDen;
}

function updateMockDenuncia(id, updateData) {
  const index = mockDenunciasData.findIndex(d => d.id == id);
  if (index === -1) throw new Error('Denúncia não encontrada');
  mockDenunciasData[index] = { ...mockDenunciasData[index], ...updateData, updatedAt: new Date().toISOString() };
  saveMockDataToStorage();
  return mockDenunciasData[index];
}

function deleteMockDenuncia(id) {
  const index = mockDenunciasData.findIndex(d => d.id == id);
  if (index === -1) throw new Error('Denúncia não encontrada');
  mockDenunciasData.splice(index, 1);
  saveMockDataToStorage();
  return { success: true };
}

function changeMockDenunciaStatus(id, status) {
  const index = mockDenunciasData.findIndex(d => d.id == id);
  if (index === -1) throw new Error('Denúncia não encontrada');
  mockDenunciasData[index] = { ...mockDenunciasData[index], status, updatedAt: new Date().toISOString() };
  saveMockDataToStorage();
  return mockDenunciasData[index];
}
...
function filterTable(section) {
  ...
  if (!TABLE_STATE[section]) {
    console.warn('Seção desconhecida para filtragem:', section);
    return;
  }

  // Atualizar filtro no estado e renderizar página 1
  TABLE_STATE[section].filter = filter;
  TABLE_STATE[section].page = 1;
  changePage(section, 1);
}
...
function clearSearch(section) {
  ...
  searchInput.value = '';
  if (TABLE_STATE[section]) {
    TABLE_STATE[section].filter = '';
    TABLE_STATE[section].page = 1;
  }
  changePage(section, 1);
}

function attachSearchClearHandlers() {
  document.querySelectorAll('.search-box').forEach(box => {
    const input = box.querySelector('input');
    const clearBtn = box.querySelector('.clear-search');
...
function renderNewsTable(news) {
  ...
  const state = TABLE_STATE.news;
  news = news || state.items || [];
  const filtered = news.filter(n => itemMatchesFilter(n, state.filter));
  const totalItems = filtered.length;
  // Update news count
  if (countElement) {
    countElement.innerHTML = `<i class="fas fa-newspaper"></i> ${totalItems} notícia${totalItems !== 1 ? 's' : ''}`;
  }
  ...
  if (!filtered || filtered.length === 0) {
    ...
    `;
    renderPaginationControls('news', 0);
    return;
  }
  const pageItems = paginateArray(filtered, state.page || 1, state.pageSize || 10);
  ...
  `).join('');
  renderPaginationControls('news', totalItems);
}
...
// 'Denúncias Recebidas' UI was removed; we keep statistics and table management.
// Note: removed functions: initializeReceivedData, loadDenuncias, renderDenuncias, filterDenuncias.

async function updateStats() {
  // Atualiza as estatísticas completas utilizando o processo de analytics já implementado
  try {
    await loadAllAnalytics();
  } catch (error) {
    // Em caso de falha, exibir no console mas continuar
    console.error('Erro ao atualizar estatísticas:', error);
  }
}

async function viewDenunciaDetails(id) {
  // Tentar buscar denúncia no cache local (se existir)
  let denunciasCache = JSON.parse(localStorage.getItem('ssp-denuncias-cache') || '[]');
  let denuncia = denunciasCache.find(d => d.id == id);

  // Se não encontrado no cache, buscar via API (fallback)
  if (!denuncia) {
    try {
      const all = await apiRequest('/denuncias');
      denuncia = all.find(d => d.id == id);
    } catch (err) {
      console.warn('Não foi possível buscar denúncia pela API:', err.message);
    }
  }
...
function renderCamerasTable(cameras) {
  ...
  const state = TABLE_STATE.cameras;
  cameras = cameras || state.items || [];
  const filtered = cameras.filter(c => itemMatchesFilter(c, state.filter));
  const totalItems = filtered.length;
  // Update cameras count
  if (countElement) {
    countElement.innerHTML = `<i class="fas fa-video"></i> ${totalItems} câmera${totalItems !== 1 ? 's' : ''}`;
  }
  ...
  if (!filtered || filtered.length === 0) {
    ...
    `;
    renderPaginationControls('cameras', totalItems);
    return;
  }
  const pageItems = paginateArray(filtered, state.page || 1, state.pageSize || 10);
  tbody.innerHTML = pageItems.map(camera => `
    <tr>
      <td>${camera.id || 'N/A'}</td>
...
function renderUsersTable(users) {
  ...
  const state = TABLE_STATE.users;
  users = users || state.items || [];
  const filtered = users.filter(u => itemMatchesFilter(u, state.filter));
  const totalItems = filtered.length;
  if (!filtered || filtered.length === 0) {
    tbody.innerHTML = `
      <tr>
    ...
    `;
    renderPaginationControls('users', totalItems);
    return;
  }
  const pageItems = paginateArray(filtered, state.page || 1, state.pageSize || 10);
...
function renderDenunciasTable(denuncias) {
  ...
  const state = TABLE_STATE.denuncias;
  denuncias = denuncias || state.items || [];
  const filtered = denuncias.filter(d => itemMatchesFilter(d, state.filter));
  const totalItems = filtered.length;
  const page = state.page || 1;
  const pageItems = paginateArray(filtered, page, state.pageSize || 10);
  ...
  renderPaginationControls('denuncias', totalItems);
}
...
function clearMockData() {
  if (confirm('Tem certeza que deseja limpar todos os dados de teste?')) {
    mockNewsData = getDefaultMockNews(80);
    mockCamerasData = getDefaultMockCameras(120);
    mockDenunciasData = getDefaultMockDenuncias(200);
    saveMockDataToStorage();
    loadNews();
